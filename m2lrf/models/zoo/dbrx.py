"""
M-2LRF Native 2-Bit Architecture: DBRX
...
class DBRXAttention(nn.Module):
    def __init__(self, config: DBRXConfig):
        ...
        self.rotary_emb = DBRXRotaryEmbedding(self.head_dim, base=config.rope_theta)
...
class DBRXDecoderLayer(nn.Module):
    def __init__(self, config: DBRXConfig):
        super().__init__()
        self.self_attn = DBRXAttention(config)
...
class DBRXModel(nn.Module):
    def __init__(self, config: DBRXConfig):
        ...
        self.layers = nn.ModuleList([DBRXDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = DBRXRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
