"""
M-2LRF Native 2-Bit Architecture: Yi
...
class YiAttention(nn.Module):
    def __init__(self, config: YiConfig):
        ...
        self.rotary_emb = YiRotaryEmbedding(self.head_dim, base=config.rope_theta)
...
class YiDecoderLayer(nn.Module):
    def __init__(self, config: YiConfig):
        super().__init__()
        self.self_attn = YiAttention(config)
        self.mlp = YiMLP(config)
        self.input_layernorm = YiRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = YiRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class YiModel(nn.Module):
    def __init__(self, config: YiConfig):
        ...
        self.layers = nn.ModuleList([YiDecoderLayer(config) for _ in range(config.num_hidden_layers)])
