"""
M-2LRF Native 2-Bit Architecture: OPT
...
class OPTRotaryEmbedding(nn.Module):
...
class OPTAttention(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.rotary_emb = OPTRotaryEmbedding(self.head_dim, base=config.rope_theta)
...
class OPTMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.down_proj(F.relu(self.gate_proj(x)) * self.up_proj(x))
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.input_layernorm = OPTRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = OPTRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class OPTModel(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.norm = OPTRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
