"""
M-2LRF Native 2-Bit Architecture: Bloom
...
class BloomRotaryEmbedding(nn.Module):
...
class BloomAttention(nn.Module):
    def __init__(self, config: BloomConfig):
        ...
        self.rotary_emb = BloomRotaryEmbedding(self.head_dim, base=config.rope_theta)
...
class BloomDecoderLayer(nn.Module):
    def __init__(self, config: BloomConfig):
        ...
        self.input_layernorm = BloomRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = BloomRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class BloomModel(nn.Module):
    def __init__(self, config: BloomConfig):
        ...
        self.layers = nn.ModuleList([BloomDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = BloomRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
