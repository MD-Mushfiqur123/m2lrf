"""
M-2LRF Native 2-Bit Architecture: Falcon
...
class FalconAttention(nn.Module):
    def __init__(self, config: FalconConfig):
        ...
        self.rotary_emb = FalconRotaryEmbedding(self.head_dim, base=config.rope_theta)
...
class FalconDecoderLayer(nn.Module):
    def __init__(self, config: FalconConfig):
        super().__init__()
        self.self_attn = FalconAttention(config)
        self.mlp = FalconMLP(config)
        self.input_layernorm = FalconRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = FalconRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class FalconModel(nn.Module):
    def __init__(self, config: FalconConfig):
        ...
        self.layers = nn.ModuleList([FalconDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = FalconRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
