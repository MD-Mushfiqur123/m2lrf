"""
M-2LRF Native 2-Bit Architecture: SmolLM
...
class SmolLMAttention(nn.Module):
    def __init__(self, config: SmolLMConfig):
        ...
        self.rotary_emb = SmolLMRotaryEmbedding(self.head_dim, base=config.rope_theta)
...
class SmolLMDecoderLayer(nn.Module):
    def __init__(self, config: SmolLMConfig):
        ...
        self.input_layernorm = SmolLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = SmolLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class SmolLMModel(nn.Module):
    def __init__(self, config: SmolLMConfig):
        ...
        self.layers = nn.ModuleList([SmolLMDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = SmolLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
