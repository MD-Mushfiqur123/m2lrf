"""
M-2LRF Native 2-Bit Architecture: Jamba
...
class JambaAttention(nn.Module):
    def __init__(self, config: JambaConfig):
        ...
        self.rotary_emb = JambaRotaryEmbedding(self.head_dim, base=config.rope_theta)
...
class JambaDecoderLayer(nn.Module):
    def __init__(self, config: JambaConfig):
        ...
        self.post_attention_layernorm = JambaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class JambaModel(nn.Module):
    def __init__(self, config: JambaConfig):
        ...
        self.layers = nn.ModuleList([JambaDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = JambaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
